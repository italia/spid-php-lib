...
class Response implements ResponseInterface
{
    ...
    public function validate($xml, $hasAssertion): bool
    {
        ...
        ? $this->saml->settings['accepted_clock_skew_seconds'] : 300;
        ...
        } elseif ($root->getAttribute('IssueInstant') == "" || strtotime($root->getAttribute('IssueInstant'))
        ...
        if ($xml->getElementsByTagName('Issuer')->length == 0) {
            ...
            throw new \Exception("Invalid Issuer attribute, expected ".
                "'urn:oasis:names:tc:SAML:2.0:nameid-format:entity'" .
                " but received " . $xml->getElementsByTagName('Issuer')->item(0)->getAttribute('Format'));
        }
        ...
        if ($hasAssertion) {
            if ($xml->getElementsByTagName('Assertion')->item(0)->getAttribute('ID') == ""
            ...
            } elseif (!$this->validateDate($xml->getElementsByTagName('Assertion')
                ->item(0)->getAttribute('IssueInstant'))) {
                ...
                || strtotime($xml->getElementsByTagName('Assertion')->item(0)->getAttribute('IssueInstant'))
                    > strtotime('now') + $accepted_clock_skew_seconds) {
                throw new \Exception("Invalid IssueInstant attribute on Assertion");
            } elseif ($xml->getElementsByTagName('Assertion')->item(0)->getAttribute('IssueInstant') == "") {
                throw new \Exception("Invalid IssueInstant attribute on Assertion");
            ...
            if ($hasAssertion && $xml->getElementsByTagName('Issuer')->item(1)->nodeValue != $_SESSION['idpEntityId']) {
                ...
                throw new \Exception("Invalid Issuer attribute, expected ".
                "'urn:oasis:names:tc:SAML:2.0:nameid-format:entity'".
                " but received " . $xml->getElementsByTagName('Issuer')->item(1)->getAttribute('Format'));
            }
            ...
            if ($xml->getElementsByTagName('Conditions')->length == 0) {
                ...
                || strtotime($xml->getElementsByTagName('Conditions')->item(0)->getAttribute('NotBefore'))
                >strtotime('now') + $accepted_clock_skew_seconds
            ) {
            ...
            } elseif (!$this->validateDate($xml->getElementsByTagName('Conditions')
                ->item(0)->getAttribute('NotBefore'))) {
                throw new \Exception("Invalid NotBefore attribute");
            } elseif (!$this->validateDate($xml->getElementsByTagName('Conditions')
                ->item(0)->getAttribute('NotOnOrAfter'))) {
                throw new \Exception("Invalid NotOnOrAfter attribute");
            }
            ...
            if ($xml->getElementsByTagName('SubjectConfirmationData')->length == 0) {
                ...
                " but received " . $xml->getElementsByTagName('SubjectConfirmation')->item(0)->getAttribute('Method'));
            }
